...
class Program
{
    ...
    static List<Debtor> debtors = new List<Debtor> {
            new Debtor("Shirley T. Qualls", DateTime.Parse("March 30, 1932"), "123-45-6789", "ShirleyTQualls@teleworm.us", "3565 Eagles Nest Drive Woodland, CA 95695", 2414),
            ...
            new Debtor("Martin S. Malinowski", DateTime.Parse("January 17, 1992"), "765-577-3523", "MartinSMalinowski@dayrep.com", "3749 Capitol Avenue New Castle, IN 47362", 1816),
    ...
    public static bool whoHaveSame3characters(Debtor d)
    {
        int say;
        for (int i = 0; i < d.FullName.Length; i++)
        {
            ...
            {
                if (d.FullName[i] == d.FullName[j]) say++;
            }
            if (say >= 3)
            {
                return true;
    ...
    public static bool whosePhoneNumberIsDifferent(Debtor d)
    {
        string number = d.Phone;
    ...
    public static bool whoCanPayDebtUntilBirthday(Debtor d)
    {
        int timeLeft = d.BirthDay.Month - DateTime.Now.Month;
        if (timeLeft < 0) timeLeft = 12 + timeLeft;
        if (d.Debt - timeLeft * 500 <= 0)
            return true;
        return false;
    }
    ...
    static void Main()
    {
        ...
        {
            averageDebt += debtor.Debt;
        }
        averageDebt /= debtors.Count;

        // 2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag
        // debtors.FindAll(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com")).Print();


        // 3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
        // debtors.FindAll(d => (2022 - d.BirthDay.Year) > 26 && (2022 - d.BirthDay.Year) < 36).Print();
        ...
        // debtors.FindAll(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') > 1).Print();


        // 7) Qishda anadan olan borclulari cixardmaq
        // debtors.FindAll(d => d.BirthDay.Month == 1 || d.BirthDay.Month > 10).Print();
        ...
        // debtors = debtors.OrderBy(d => d.getSurname()).ToList();
        // debtors = debtors.OrderByDescending(d => d.Debt).ToList();
        // debtors.FindAll(d => d.Debt > averageDebt).Print();



        // 9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
        // debtors.FindAll(d => !d.Phone.Contains('8')).ForEach(d=>Console.WriteLine($"{d.getSurname()}, {2022-d.BirthDay.Year}, {d.Debt}"));
        ...
        // debtors = debtors.OrderBy(d => d.FullName).ToList();
        // Predicate<Debtor> p = whoHaveSame3characters;
        // debtors.FindAll(p).Print();
        ...
        // {
        //     yearCount = 0;
        //     year = debtor.BirthDay.Year;
        //     foreach (var debtor2 in debtors)
        //     {
        //         if (debtor.BirthDay.Year == debtor2.BirthDay.Year)
        //             yearCount++;
        //     }
        //     if (yearCount > mostBirthYear) mostBirthYear = year;
        // }
        // Console.WriteLine(mostBirthYear);
        ...
        // double allDebt = 0;
        // debtors.ForEach(d=>allDebt+=d.Debt);
        ...
        // Predicate<Debtor> p = whosePhoneNumberIsDifferent;
        // debtors.FindAll(p).Print();
        ...
        // debtors.FindAll(whoCanPayDebtUntilBirthday).Print();
        ...
        //     d.FullName.ToLower().Contains('m') &&
        //     d.FullName.ToLower().Contains('i') &&
        //     d.FullName.ToLower().Contains('l') &&
        //     d.FullName.ToLower().Contains('e')).Print();
